...
async def handle_text_message(message: types.Message):
    ...
    if (
        ...
        await download_story_for_api(text, chat_id)  # download story for link
    elif message.text.startswith("@"):
        await download_profilePic(
            message, chat_id
        )  # download prfile picture for nickname
        ...
        await downloadPostLink(message, chat_id, text)  # download post for link
    else:
            ...
            for url_item in urls:
                ...
                if url_item["name"] == "JPG":
                    ...
                    if file_path:
                        await send_file(
                            chat_id, file_path
                        )  # Здесь вместо CHAT_ID укажите ID чата, куда вы хотите отправить файл
    except:
        await message.reply(
